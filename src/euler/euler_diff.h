...
void RoeSolver_diff( Tsol* q, Tsol* qg, Tres* aux_vars, Tmsh* nrm,
                     Tres* fluxL_dot, Tres* fluxR_dot);

template <typename Tsol, typename Tmsh, typename Tres>
void calcSAT_diff(Tsol roe_vars[2], Tsol roe_vars_dot[15], Tsol* dq, 
...
//  fac_dot1 = -d1_0/(q[0]*q[0])

  auto uL = q[1]*fac;
  auto uL_dot1 = q[1]*fac_dot1;
  auto uL_dot2 = fac;

  auto vL = q[2]*fac;
  auto vL_dot1 = q[2]*fac_dot1;
  auto vL_dot3 = fac;
...
//  fac_dot1 = -d1_0/(qg[0]*qg[0])
  ...
  auto HR_dot1 = gamma*qg[3]*fac_dot1 - gami*phi_dot1;
...
//  roe_vars_dot = params.roe_vars_dot
  Tsol roe_vars_dot[15];
  roe_vars_dot[0]  = u_dotL1;
  roe_vars_dot[1]  = u_dotR1;
  roe_vars_dot[2]  = u_dotL2;
  roe_vars_dot[3]  = u_dotR2;

  roe_vars_dot[4]  = v_dotL1;
  roe_vars_dot[5]  = v_dotR1;
  roe_vars_dot[6]  = v_dotL3;
  roe_vars_dot[7]  = v_dotR3;

  roe_vars_dot[8]  = H_dotL1;
  roe_vars_dot[9] = H_dotR1;
  roe_vars_dot[10] = H_dotL2;
  roe_vars_dot[11] = H_dotR2;
  roe_vars_dot[12] = H_dotL3;
  roe_vars_dot[13] = H_dotR3;
  roe_vars_dot[14] = H_dotL4;
  roe_vars_dot[15] = H_dotR4;
...
//  sat_jacR = params.sat_jacR
  ...
  // contribution below
  calcSAT_diff(roe_vars, roe_vars_dot,  dq, nrm, fluxL_dot, fluxR_dot);
...
//  convertFromNaturalToWorkingVars(params, q, v_vals)
  calcEulerFlux_diff(q, aux_vars, nrm2, euler_fluxjac);
...
// into a vector
  ...
  auto u_dotL1 = roe_vars_dot[0];
  auto u_dotR1 = roe_vars_dot[1];
  auto u_dotL2 = roe_vars_dot[2];
  auto u_dotR2 = roe_vars_dot[3];

  auto v_dotL1 = roe_vars_dot[4];
  auto v_dotR1 = roe_vars_dot[5];
  auto v_dotL3 = roe_vars_dot[6];
  auto v_dotR3 = roe_vars_dot[7];

  auto H_dotL1 = roe_vars_dot[8];
  auto H_dotR1 = roe_vars_dot[9];
  auto H_dotL2 = roe_vars_dot[10];
  auto H_dotR2 = roe_vars_dot[11];
  auto H_dotL3 = roe_vars_dot[12];
  auto H_dotR3 = roe_vars_dot[13];
  auto H_dotL4 = roe_vars_dot[14];
  auto H_dotR4 = roe_vars_dot[15];
...
//    println("not using lambda1")
  ...
  Tres lambda3_dotL4, lambda3_dotR4;
...
//  @printit lambda1 lambda2 lambda3 lambda1_dotR1 lambda2_dotR1 lambda3_dotR1
  ...
  // sat[0] = lambda3*dq1
  sat_jacL[ind(0, 0, 4, 4)] = lambda3 + dq1*lambda3_dotL1;
  sat_jacL[ind(0, 1, 4, 4)] =         + dq1*lambda3_dotL2;
  sat_jacL[ind(0, 2, 4, 4)] =         + dq1*lambda3_dotL3;
  sat_jacL[ind(0, 3, 4, 4)] =         + dq1*lambda3_dotL4;
  //TODO: zero out unused entries?

  sat_jacR[ind(0, 0, 4, 4)] = -lambda3 + dq1*lambda3_dotR1;
  sat_jacR[ind(0, 1, 4, 4)] =          + dq1*lambda3_dotR2;
  sat_jacR[ind(0, 2, 4, 4)] =          + dq1*lambda3_dotR3;
  sat_jacR[ind(0, 3, 4, 4)] =          + dq1*lambda3_dotR4;

  // sat[1] = lambda3*dq2
  sat_jacL[ind(1, 0, 4, 4)] =         + dq2*lambda3_dotL1;
  sat_jacL[ind(1, 1, 4, 4)] = lambda3 + dq2*lambda3_dotL2;
  sat_jacL[ind(1, 2, 4, 4)] =         + dq2*lambda3_dotL3;
  sat_jacL[ind(1, 3, 4, 4)] =         + dq2*lambda3_dotL4;

  sat_jacR[ind(1, 0, 4, 4)] =          + dq2*lambda3_dotR1;
  sat_jacR[ind(1, 1, 4, 4)] = -lambda3 + dq2*lambda3_dotR2;
  sat_jacR[ind(1, 2, 4, 4)] =          + dq2*lambda3_dotR3;
  sat_jacR[ind(1, 3, 4, 4)] =          + dq2*lambda3_dotR4;

  // sat[2] = lambda3*dq3
  sat_jacL[ind(2, 0, 4, 4)] =         + dq3*lambda3_dotL1;
  sat_jacL[ind(2, 1, 4, 4)] =         + dq3*lambda3_dotL2;
  sat_jacL[ind(2, 2, 4, 4)] = lambda3 + dq3*lambda3_dotL3;
  sat_jacL[ind(2, 3, 4, 4)] =         + dq3*lambda3_dotL4;

  sat_jacR[ind(2, 0, 4, 4)] =          + dq3*lambda3_dotR1;
  sat_jacR[ind(2, 1, 4, 4)] =          + dq3*lambda3_dotR2;
  sat_jacR[ind(2, 2, 4, 4)] = -lambda3 + dq3*lambda3_dotR3;
  sat_jacR[ind(2, 3, 4, 4)] =          + dq3*lambda3_dotR4;

  // sat[3] = lambda3*dq4;
  sat_jacL[ind(3, 0, 4, 4)] =           dq4*lambda3_dotL1;
  sat_jacL[ind(3, 1, 4, 4)] =           dq4*lambda3_dotL2;
  sat_jacL[ind(3, 2, 4, 4)] =           dq4*lambda3_dotL3;
  sat_jacL[ind(3, 3, 4, 4)] = lambda3 + dq4*lambda3_dotL4;

  sat_jacR[ind(3, 0, 4, 4)] =            dq4*lambda3_dotR1;
  sat_jacR[ind(3, 1, 4, 4)] =            dq4*lambda3_dotR2;
  sat_jacR[ind(3, 2, 4, 4)] =            dq4*lambda3_dotR3;
  sat_jacR[ind(3, 3, 4, 4)] = -lambda3 + dq4*lambda3_dotR4;

//  @printit sat_jacR[ind(1, 0, 4, 4)]
...
//  E2dq = params.res_vals2
  ...
  E1dq[1] = E1dq[0]*u;
  auto E1dq2_dotL1 = u*E1dq1_dotL1 + E1dq[0]*u_dotL1;
  auto E1dq2_dotL2 = u*E1dq1_dotL2 + E1dq[0]*u_dotL2;
  auto E1dq2_dotL3 = u*E1dq1_dotL3;
  auto E1dq2_dotL4 = u*E1dq1_dotL4;

  auto E1dq2_dotR1 = u*E1dq1_dotR1 + E1dq[0]*u_dotR1;
  auto E1dq2_dotR2 = u*E1dq1_dotR2 + E1dq[0]*u_dotR2;
  auto E1dq2_dotR3 = u*E1dq1_dotR3;
  auto E1dq2_dotR4 = u*E1dq1_dotR4;

  E1dq[2] = E1dq[0]*v;
  auto E1dq3_dotL1 = v*E1dq1_dotL1 + E1dq[0]*v_dotL1;
  auto E1dq3_dotL2 = v*E1dq1_dotL2;
  auto E1dq3_dotL3 = v*E1dq1_dotL3 + E1dq[0]*v_dotL3;
  auto E1dq3_dotL4 = v*E1dq1_dotL4;

  auto E1dq3_dotR1 = v*E1dq1_dotR1 + E1dq[0]*v_dotR1;
  auto E1dq3_dotR2 = v*E1dq1_dotR2;
  auto E1dq3_dotR3 = v*E1dq1_dotR3 + E1dq[0]*v_dotR3;
  auto E1dq3_dotR4 = v*E1dq1_dotR4;

  E1dq[3] = E1dq[0]*H;
  auto E1dq4_dotL1 = H*E1dq1_dotL1 + E1dq[0]*H_dotL1;
  auto E1dq4_dotL2 = H*E1dq1_dotL2 + E1dq[0]*H_dotL2;
  auto E1dq4_dotL3 = H*E1dq1_dotL3 + E1dq[0]*H_dotL3;
  auto E1dq4_dotL4 = H*E1dq1_dotL4 + E1dq[0]*H_dotL4;

  auto E1dq4_dotR1 = H*E1dq1_dotR1 + E1dq[0]*H_dotR1;
  auto E1dq4_dotR2 = H*E1dq1_dotR2 + E1dq[0]*H_dotR2;
  auto E1dq4_dotR3 = H*E1dq1_dotR3 + E1dq[0]*H_dotR3;
  auto E1dq4_dotR4 = H*E1dq1_dotR4 + E1dq[0]*H_dotR4;
  ...
  E2dq[3] = E2dq[1]*Un;
  auto E2dq4_dotL1 = Un*E2dq2_dotL1 + E2dq[1]*Un_dotL1;
  auto E2dq4_dotL2 = Un*E2dq2_dotL2 + E2dq[1]*Un_dotL2;
  auto E2dq4_dotL3 = Un*E2dq2_dotL3 + E2dq[1]*Un_dotL3;

  auto E2dq4_dotR1 = Un*E2dq2_dotR1 + E2dq[1]*Un_dotR1;
  auto E2dq4_dotR2 = Un*E2dq2_dotR2 + E2dq[1]*Un_dotR2;
  auto E2dq4_dotR3 = Un*E2dq2_dotR3 + E2dq[1]*Un_dotR3;
...
//  @printit tmp1 tmp2 E1dq1_dotR1 E1dq[0] tmp1_dotR1 tmp2_dotR1 tmp3 E2dq[0]

  //TODO: align + signs
  sat_jacL[ind(0, 0, 4, 4)] += tmp1*tmp2*E1dq1_dotL1   + tmp1*E1dq[0]*tmp2_dotL1 + 
                   tmp2*E1dq[0]*tmp1_dotL1 + 
                                           + tmp3*E2dq[0]*tmp1_dotL1;

  sat_jacL[ind(0, 1, 4, 4)] += tmp1*tmp2*E1dq1_dotL2   + tmp1*E1dq[0]*tmp2_dotL2 + 
                   tmp2*E1dq[0]*tmp1_dotL2 + 
                                           + tmp3*E2dq[0]*tmp1_dotL2;

  sat_jacL[ind(0, 2, 4, 4)] += tmp1*tmp2*E1dq1_dotL3   + tmp1*E1dq[0]*tmp2_dotL3 + 
                   tmp2*E1dq[0]*tmp1_dotL3 + 
                                           + tmp3*E2dq[0]*tmp1_dotL3;

  sat_jacL[ind(0, 3, 4, 4)] += tmp1*tmp2*E1dq1_dotL4   + tmp1*E1dq[0]*tmp2_dotL4 + 
                   tmp2*E1dq[0]*tmp1_dotL4 + 
                                           + tmp3*E2dq[0]*tmp1_dotL4;

  sat_jacR[ind(0, 0, 4, 4)] += tmp1*tmp2*E1dq1_dotR1   + tmp1*E1dq[0]*tmp2_dotR1 + 
                   tmp2*E1dq[0]*tmp1_dotR1 + 
                                           + tmp3*E2dq[0]*tmp1_dotR1;

  sat_jacR[ind(0, 1, 4, 4)] += tmp1*tmp2*E1dq1_dotR2   + tmp1*E1dq[0]*tmp2_dotR2 + 
                   tmp2*E1dq[0]*tmp1_dotR2 + 
                                           + tmp3*E2dq[0]*tmp1_dotR2;

  sat_jacR[ind(0, 2, 4, 4)] += tmp1*tmp2*E1dq1_dotR3   + tmp1*E1dq[0]*tmp2_dotR3 + 
                   tmp2*E1dq[0]*tmp1_dotR3 + 
                                           + tmp3*E2dq[0]*tmp1_dotR3;

  sat_jacR[ind(0, 3, 4, 4)] += tmp1*tmp2*E1dq1_dotR4   + tmp1*E1dq[0]*tmp2_dotR4 + 
                   tmp2*E1dq[0]*tmp1_dotR4 + 
                                           + tmp3*E2dq[0]*tmp1_dotR4;


  sat_jacL[ind(1, 0, 4, 4)] += tmp1*tmp2*E1dq2_dotL1   + tmp1*E1dq[1]*tmp2_dotL1 + 
                   tmp2*E1dq[1]*tmp1_dotL1 + 
                   tmp1*tmp3*E2dq2_dotL1   + tmp3*E2dq[1]*tmp1_dotL1;

  sat_jacL[ind(1, 1, 4, 4)] += tmp1*tmp2*E1dq2_dotL2   + tmp1*E1dq[1]*tmp2_dotL2 + 
                   tmp2*E1dq[1]*tmp1_dotL2 + 
                   tmp1*tmp3*E2dq2_dotL2   + tmp3*E2dq[1]*tmp1_dotL2;

  sat_jacL[ind(1, 2, 4, 4)] += tmp1*tmp2*E1dq2_dotL3   + tmp1*E1dq[1]*tmp2_dotL3 + 
                   tmp2*E1dq[1]*tmp1_dotL3 + 
                   tmp1*tmp3*E2dq2_dotL3   + tmp3*E2dq[1]*tmp1_dotL3;

  sat_jacL[ind(1, 3, 4, 4)] += tmp1*tmp2*E1dq2_dotL4   + tmp1*E1dq[1]*tmp2_dotL4 + 
                   tmp2*E1dq[1]*tmp1_dotL4 + 
                                           + tmp3*E2dq[1]*tmp1_dotL4;

  sat_jacR[ind(1, 0, 4, 4)] += tmp1*tmp2*E1dq2_dotR1   + tmp1*E1dq[1]*tmp2_dotR1 + 
                   tmp2*E1dq[1]*tmp1_dotR1 + 
                   tmp1*tmp3*E2dq2_dotR1   + tmp3*E2dq[1]*tmp1_dotR1;

  sat_jacR[ind(1, 1, 4, 4)] += tmp1*tmp2*E1dq2_dotR2   + tmp1*E1dq[1]*tmp2_dotR2 + 
                   tmp2*E1dq[1]*tmp1_dotR2 + 
                   tmp1*tmp3*E2dq2_dotR2   + tmp3*E2dq[1]*tmp1_dotR2;

  sat_jacR[ind(1, 2, 4, 4)] += tmp1*tmp2*E1dq2_dotR3   + tmp1*E1dq[1]*tmp2_dotR3 + 
                   tmp2*E1dq[1]*tmp1_dotR3 + 
                   tmp1*tmp3*E2dq2_dotR3   + tmp3*E2dq[1]*tmp1_dotR3;

  sat_jacR[ind(1, 3, 4, 4)] += tmp1*tmp2*E1dq2_dotR4   + tmp1*E1dq[1]*tmp2_dotR4 + 
                   tmp2*E1dq[1]*tmp1_dotR4 + 
                                           + tmp3*E2dq[1]*tmp1_dotR4;


  sat_jacL[ind(2, 0, 4, 4)] += tmp1*tmp2*E1dq3_dotL1   + tmp1*E1dq[2]*tmp2_dotL1 + 
                   tmp2*E1dq[2]*tmp1_dotL1 + 
                   tmp1*tmp3*E2dq3_dotL1   + tmp3*E2dq[2]*tmp1_dotL1;

  sat_jacL[ind(2, 1, 4, 4)] += tmp1*tmp2*E1dq3_dotL2   + tmp1*E1dq[2]*tmp2_dotL2 + 
                   tmp2*E1dq[2]*tmp1_dotL2 + 
                   tmp1*tmp3*E2dq3_dotL2   + tmp3*E2dq[2]*tmp1_dotL2;

  sat_jacL[ind(2, 2, 4, 4)] += tmp1*tmp2*E1dq3_dotL3   + tmp1*E1dq[2]*tmp2_dotL3 + 
                   tmp2*E1dq[2]*tmp1_dotL3 + 
                   tmp1*tmp3*E2dq3_dotL3   + tmp3*E2dq[2]*tmp1_dotL3;

  sat_jacL[ind(2, 3, 4, 4)] += tmp1*tmp2*E1dq3_dotL4   + tmp1*E1dq[2]*tmp2_dotL4 + 
                   tmp2*E1dq[2]*tmp1_dotL4 + 
                                           + tmp3*E2dq[2]*tmp1_dotL4;

  sat_jacR[ind(2, 0, 4, 4)] += tmp1*tmp2*E1dq3_dotR1   + tmp1*E1dq[2]*tmp2_dotR1 + 
                   tmp2*E1dq[2]*tmp1_dotR1 + 
                   tmp1*tmp3*E2dq3_dotR1   + tmp3*E2dq[2]*tmp1_dotR1;

  sat_jacR[ind(2, 1, 4, 4)] += tmp1*tmp2*E1dq3_dotR2   + tmp1*E1dq[2]*tmp2_dotR2 + 
                   tmp2*E1dq[2]*tmp1_dotR2 + 
                   tmp1*tmp3*E2dq3_dotR2   + tmp3*E2dq[2]*tmp1_dotR2;

  sat_jacR[ind(2, 2, 4, 4)] += tmp1*tmp2*E1dq3_dotR3   + tmp1*E1dq[2]*tmp2_dotR3 + 
                   tmp2*E1dq[2]*tmp1_dotR3 + 
                   tmp1*tmp3*E2dq3_dotR3   + tmp3*E2dq[2]*tmp1_dotR3;

  sat_jacR[ind(2, 3, 4, 4)] += tmp1*tmp2*E1dq3_dotR4   + tmp1*E1dq[2]*tmp2_dotR4 + 
                   tmp2*E1dq[2]*tmp1_dotR4 + 
                                           + tmp3*E2dq[2]*tmp1_dotR4;


  sat_jacL[ind(3, 0, 4, 4)] += tmp1*tmp2*E1dq4_dotL1   + tmp1*E1dq[3]*tmp2_dotL1 + 
                   tmp2*E1dq[3]*tmp1_dotL1 + 
                   tmp1*tmp3*E2dq4_dotL1   + tmp3*E2dq[3]*tmp1_dotL1;

  sat_jacL[ind(3, 1, 4, 4)] += tmp1*tmp2*E1dq4_dotL2   + tmp1*E1dq[3]*tmp2_dotL2 + 
                   tmp2*E1dq[3]*tmp1_dotL2 + 
                   tmp1*tmp3*E2dq4_dotL2   + tmp3*E2dq[3]*tmp1_dotL2;

  sat_jacL[ind(3, 2, 4, 4)] += tmp1*tmp2*E1dq4_dotL3   + tmp1*E1dq[3]*tmp2_dotL3 + 
                   tmp2*E1dq[3]*tmp1_dotL3 + 
                   tmp1*tmp3*E2dq4_dotL3   + tmp3*E2dq[3]*tmp1_dotL3;

  sat_jacL[ind(3, 3, 4, 4)] += tmp1*tmp2*E1dq4_dotL4   + tmp1*E1dq[3]*tmp2_dotL4 + 
                   tmp2*E1dq[3]*tmp1_dotL4 + 
                                           + tmp3*E2dq[3]*tmp1_dotL4;

  sat_jacR[ind(3, 0, 4, 4)] += tmp1*tmp2*E1dq4_dotR1   + tmp1*E1dq[3]*tmp2_dotR1 + 
                   tmp2*E1dq[3]*tmp1_dotR1 + 
                   tmp1*tmp3*E2dq4_dotR1   + tmp3*E2dq[3]*tmp1_dotR1;

  sat_jacR[ind(3, 1, 4, 4)] += tmp1*tmp2*E1dq4_dotR2   + tmp1*E1dq[3]*tmp2_dotR2 + 
                   tmp2*E1dq[3]*tmp1_dotR2 + 
                   tmp1*tmp3*E2dq4_dotR2   + tmp3*E2dq[3]*tmp1_dotR2;

  sat_jacR[ind(3, 2, 4, 4)] += tmp1*tmp2*E1dq4_dotR3   + tmp1*E1dq[3]*tmp2_dotR3 + 
                   tmp2*E1dq[3]*tmp1_dotR3 + 
                   tmp1*tmp3*E2dq4_dotR3   + tmp3*E2dq[3]*tmp1_dotR3;

  sat_jacR[ind(3, 3, 4, 4)] += tmp1*tmp2*E1dq4_dotR4   + tmp1*E1dq[3]*tmp2_dotR4 + 
...
//  @printit sat_jacR[ind(1, 0, 4, 4)]
  ...
  E1dq[1] = E1dq[0]*u;
  E1dq2_dotL1 = u*E1dq1_dotL1 + E1dq[0]*u_dotL1;
  E1dq2_dotL2 = u*E1dq1_dotL2 + E1dq[0]*u_dotL2;
  E1dq2_dotL3 = u*E1dq1_dotL3;

  E1dq2_dotR1 = u*E1dq1_dotR1 + E1dq[0]*u_dotR1;
  E1dq2_dotR2 = u*E1dq1_dotR2 + E1dq[0]*u_dotR2;
  E1dq2_dotR3 = u*E1dq1_dotR3;


  E1dq[2] = E1dq[0]*v;
  E1dq3_dotL1 = v*E1dq1_dotL1 + E1dq[0]*v_dotL1;
  E1dq3_dotL2 = v*E1dq1_dotL2;
  E1dq3_dotL3 = v*E1dq1_dotL3 + E1dq[0]*v_dotL3;

  E1dq3_dotR1 = v*E1dq1_dotR1 + E1dq[0]*v_dotR1;
  E1dq3_dotR2 = v*E1dq1_dotR2;
  E1dq3_dotR3 = v*E1dq1_dotR3 + E1dq[0]*v_dotR3;


  E1dq[3] = E1dq[0]*H;
  E1dq4_dotL1 = H*E1dq1_dotL1 + E1dq[0]*H_dotL1;
  E1dq4_dotL2 = H*E1dq1_dotL2 + E1dq[0]*H_dotL2;
  E1dq4_dotL3 = H*E1dq1_dotL3 + E1dq[0]*H_dotL3;
  E1dq4_dotL4 =               + E1dq[0]*H_dotL4;

  E1dq4_dotR1 = H*E1dq1_dotR1 + E1dq[0]*H_dotR1;
  E1dq4_dotR2 = H*E1dq1_dotR2 + E1dq[0]*H_dotR2;
  E1dq4_dotR3 = H*E1dq1_dotR3 + E1dq[0]*H_dotR3;
  E1dq4_dotR4 =               + E1dq[0]*H_dotR4;


  //-- get E4*dq
  t1 = phi*dq1 - u*dq2 - v*dq3 + dq4;
  auto t1_dotL1 = phi + dq1*phi_dotL1     - dq2*u_dotL1     - dq3*v_dotL1;
  auto t1_dotL2 =       dq1*phi_dotL2 - u - dq2*u_dotL2;
  auto t1_dotL3 =       dq1*phi_dotL3                   - v - dq3*v_dotL3;
  auto t1_dotL4 = 1;

  auto t1_dotR1 = -phi + dq1*phi_dotR1     - dq2*u_dotR1     - dq3*v_dotR1;
  auto t1_dotR2 =        dq1*phi_dotR2 + u - dq2*u_dotR2;
  auto t1_dotR3 =        dq1*phi_dotR3                   + v - dq3*v_dotR3;
  auto t1_dotR4 = -1;
...
//  @printit E1dq1_dotR1 E1dq2_dotR1 E1dq3_dotR1 E1dq4_dotR1 E2dq2_dotR1 E2dq3_dotR1 E2dq4_dotR1
  ...
  t1 = E1dq[0] + gami*E2dq[0];
  sat_jacL[ind(0, 0, 4, 4)] += tmp1*E1dq1_dotL1 + t1*tmp1_dotL1;
  sat_jacL[ind(0, 1, 4, 4)] += tmp1*E1dq1_dotL2 + t1*tmp1_dotL2;
  sat_jacL[ind(0, 2, 4, 4)] += tmp1*E1dq1_dotL3 + t1*tmp1_dotL3;
  sat_jacL[ind(0, 3, 4, 4)] +=                    t1*tmp1_dotL4;

  sat_jacR[ind(0, 0, 4, 4)] += tmp1*E1dq1_dotR1 + t1*tmp1_dotR1;
  sat_jacR[ind(0, 1, 4, 4)] += tmp1*E1dq1_dotR2 + t1*tmp1_dotR2;
  sat_jacR[ind(0, 2, 4, 4)] += tmp1*E1dq1_dotR3 + t1*tmp1_dotR3;
  sat_jacR[ind(0, 3, 4, 4)] +=                    t1*tmp1_dotR4;

  t1 = E1dq[1] + gami*E2dq[1];
  sat_jacL[ind(1, 0, 4, 4)] += tmp1*(E1dq2_dotL1 + gami*E2dq2_dotL1) + t1*tmp1_dotL1;
  sat_jacL[ind(1, 1, 4, 4)] += tmp1*(E1dq2_dotL2 + gami*E2dq2_dotL2) + t1*tmp1_dotL2;
  sat_jacL[ind(1, 2, 4, 4)] += tmp1*(E1dq2_dotL3 + gami*E2dq2_dotL3) + t1*tmp1_dotL3;
  sat_jacL[ind(1, 3, 4, 4)] += tmp1*(            + gami*E2dq2_dotL4) + t1*tmp1_dotL4;

  sat_jacR[ind(1, 0, 4, 4)] += tmp1*(E1dq2_dotR1 + gami*E2dq2_dotR1) + t1*tmp1_dotR1;
  sat_jacR[ind(1, 1, 4, 4)] += tmp1*(E1dq2_dotR2 + gami*E2dq2_dotR2) + t1*tmp1_dotR2;
  sat_jacR[ind(1, 2, 4, 4)] += tmp1*(E1dq2_dotR3 + gami*E2dq2_dotR3) + t1*tmp1_dotR3;
  sat_jacR[ind(1, 3, 4, 4)] += tmp1*(            + gami*E2dq2_dotR4) + t1*tmp1_dotR4;

  t1 = E1dq[2] + gami*E2dq[2];
  sat_jacL[ind(2, 0, 4, 4)] += tmp1*(E1dq3_dotL1 + gami*E2dq3_dotL1) + t1*tmp1_dotL1;
  sat_jacL[ind(2, 1, 4, 4)] += tmp1*(E1dq3_dotL2 + gami*E2dq3_dotL2) + t1*tmp1_dotL2;
  sat_jacL[ind(2, 2, 4, 4)] += tmp1*(E1dq3_dotL3 + gami*E2dq3_dotL3) + t1*tmp1_dotL3;
  sat_jacL[ind(2, 3, 4, 4)] += tmp1*(            + gami*E2dq3_dotL4) + t1*tmp1_dotL4;

  sat_jacR[ind(2, 0, 4, 4)] += tmp1*(E1dq3_dotR1 + gami*E2dq3_dotR1) + t1*tmp1_dotR1;
  sat_jacR[ind(2, 1, 4, 4)] += tmp1*(E1dq3_dotR2 + gami*E2dq3_dotR2) + t1*tmp1_dotR2;
  sat_jacR[ind(2, 2, 4, 4)] += tmp1*(E1dq3_dotR3 + gami*E2dq3_dotR3) + t1*tmp1_dotR3;
  sat_jacR[ind(2, 3, 4, 4)] += tmp1*(            + gami*E2dq3_dotR4) + t1*tmp1_dotR4;

  t1 = E1dq[3] + gami*E2dq[3];
  sat_jacL[ind(3, 0, 4, 4)] += tmp1*(E1dq4_dotL1 + gami*E2dq4_dotL1) + t1*tmp1_dotL1;
  sat_jacL[ind(3, 1, 4, 4)] += tmp1*(E1dq4_dotL2 + gami*E2dq4_dotL2) + t1*tmp1_dotL2;
  sat_jacL[ind(3, 2, 4, 4)] += tmp1*(E1dq4_dotL3 + gami*E2dq4_dotL3) + t1*tmp1_dotL3;
  sat_jacL[ind(3, 3, 4, 4)] += tmp1*(E1dq4_dotL4 + gami*E2dq4_dotL4) + t1*tmp1_dotL4;

  sat_jacR[ind(3, 0, 4, 4)] += tmp1*(E1dq4_dotR1 + gami*E2dq4_dotR1) + t1*tmp1_dotR1;
  sat_jacR[ind(3, 1, 4, 4)] += tmp1*(E1dq4_dotR2 + gami*E2dq4_dotR2) + t1*tmp1_dotR2;
  sat_jacR[ind(3, 2, 4, 4)] += tmp1*(E1dq4_dotR3 + gami*E2dq4_dotR3) + t1*tmp1_dotR3;
  sat_jacR[ind(3, 3, 4, 4)] += tmp1*(E1dq4_dotR4 + gami*E2dq4_dotR4) + t1*tmp1_dotR4;
...
//  p_dot = params.p_dot
  Tsol p_dot[4];
  auto press = calcPressure_diff(q, p_dot);

  auto fac = 1/q[0];
  auto U = (q[1]*dir[0] + q[2]*dir[1])*fac;
  auto U_dot1 = -(q[1]*dir[0] + q[2]*dir[1])*fac*fac;
  auto U_dot2 = dir[0]*fac;
  auto U_dot3 = dir[1]*fac;

  // F[0] = q[0]*U;
  // F[1] = q[1]*U + dir[0]*press;
  // F[2] = q[2]*U + dir[1]*press;
  // F[3] = (q[3] + press)*U;
  Fjac[ind(0, 0, 4, 4)] = U + q[0]*U_dot1;
  Fjac[ind(1, 0, 4, 4)] =     q[1]*U_dot1 + dir[0]*p_dot[0];
  Fjac[ind(2, 0, 4, 4)] =     q[2]*U_dot1 + dir[1]*p_dot[0];
  Fjac[ind(3, 0, 4, 4)] =     q[3]*U_dot1 + press*U_dot1 + U*p_dot[0];

  Fjac[ind(0, 1, 4, 4)] =     q[0]*U_dot2;
  Fjac[ind(1, 1, 4, 4)] = U + q[1]*U_dot2 + dir[0]*p_dot[1];
  Fjac[ind(2, 1, 4, 4)] =     q[2]*U_dot2 + dir[1]*p_dot[1];
  Fjac[ind(3, 1, 4, 4)] =     q[3]*U_dot2 + press*U_dot2 + U*p_dot[1];

  Fjac[ind(0, 2, 4, 4)] =     q[0]*U_dot3;
  Fjac[ind(1, 2, 4, 4)] =     q[1]*U_dot3 + dir[0]*p_dot[2];
  Fjac[ind(2, 2, 4, 4)] = U + q[2]*U_dot3 + dir[1]*p_dot[2];
  Fjac[ind(3, 2, 4, 4)] =     q[3]*U_dot3 + press*U_dot3 + U*p_dot[2];

  Fjac[ind(0, 3, 4, 4)] = 0;
  Fjac[ind(1, 3, 4, 4)] = dir[0]*p_dot[3];
  Fjac[ind(2, 3, 4, 4)] = dir[1]*p_dot[3];
  Fjac[ind(3, 3, 4, 4)] = U + U*p_dot[3];
}
...
Tsol calcPressure_diff(Tsol* q, Tsol* p_dot)
{
  ...
  p_dot[0] = (gamma_1)*( 0.5*(t2*t1 + t3*t1));
  p_dot[1] = -(gamma_1)*(q[1]/q[0]);
  p_dot[2] = -(gamma_1)*(q[2]/q[0]);
  p_dot[3] =   gamma_1;
  return  (gamma_1)*(q[3] - 0.5*(t2 + t3)/q[0]);
}
